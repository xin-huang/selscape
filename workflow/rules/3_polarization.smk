...
rule concat_polarized_1pop_exonic_data:
    input:
        vcfs=lambda wc: expand(
            rules.polarize_1pop_exonic_data.output.vcf,
...
rule concat_polarized_2pop_exonic_data:
    input:
        vcfs=lambda wc: expand(
            rules.polarize_2pop_exonic_data.output.vcf,
            i=get_chromosomes(wc),
            ref_genome=get_ref_genome(wc),
            allow_missing=True,
        ),
...
rule test_hwe_polarized:
    ...
    shell:
        """
        bcftools annotate --set-id '%CHROM:%POS' {input.vcf} | plink --vcf /dev/stdin --hardy --out {params.output_prefix} 2> {log}
